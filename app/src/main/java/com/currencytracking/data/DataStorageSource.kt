package com.currencytracking.data
...
class DataStorageSource(context: Context) {
    ...
    fun setBaseCurrencyCode(name: String) {
        with(preferences.edit()) {
            putString(PREFERENCE_BASE_CODE, name)
            apply()
        }
        _baseCurrencyCode.tryEmit(name)
    }
    ...
    companion object {
        ...
        private const val PREFERENCES_NAME = "EXC_RATE_PREF"
        private const val PREFERENCE_BASE_CODE = "PREF_BASE_CODE"
        private const val PREFERENCE_SORT_TYPE = "PREF_SORT_TYPE"
        private const val DEFAULT_BASE_VALUE = "USD"
    }
